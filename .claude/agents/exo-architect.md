---
name: exo-architect
description: Architecture and hard API decisions for exosnap. Use when facing design questions, complex API integrations (NVENC, WASAPI, Media Foundation, Windows App SDK), scope conflicts, or when a previous agent attempt has failed a validation gate. Produces implementation briefs for other agents. Read-first; writes files only when explicitly asked.

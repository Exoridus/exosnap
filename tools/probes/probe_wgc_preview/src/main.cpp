...
static LONG WINAPI UnhandledExceptionFilterProc(_In_ EXCEPTION_POINTERS* exceptionInfo) {
    ...
    wchar_t buf[256];
    swprintf(buf, 256, L"[probe] FATAL: unhandled SEH exception code=0x%08lX address=%p\n", code, addr);
...
int main() {
    ...
    try {
        auto targets = wgc_preview::Probe::EnumerateTargets();
        ...
        for (std::size_t i = 0; i < targets.size(); ++i) {
            auto kindStr = targets[i].kind == wgc_preview::CaptureTarget::Kind::Monitor ? L"MONITOR" : L"WINDOW";
        ...
        auto kindStr = target.kind == wgc_preview::CaptureTarget::Kind::Monitor ? L"monitor" : L"window";
        wprintf(L"\n[probe] target selected: index=%d kind=%s desc=\"%s\"\n", selectedIndex, kindStr,
                target.description.c_str());

        wgc_preview::Probe probe(target);

        if (!probe.Start()) {
            fprintf(stderr, "[probe] Start() returned false -- probe aborted\n");
            fflush(stderr);
            return 1;
        }

...
bool WgcNvencGpuProbe::Phase10_CreateNv12Texture() {
    ...
    char detail[80];
    snprintf(detail, sizeof(detail), "NV12 texture: %ux%u, D3D11_USAGE_DEFAULT, BIND_RENDER_TARGET", m_encodeWidth,
             m_encodeHeight);
    PrintDetail(detail);
...
bool WgcNvencGpuProbe::Phase11_ConfigureVideoProcessor() {
    ...
    hr = m_videoDevice->CreateVideoProcessorOutputView(m_nv12Texture.get(), m_videoEnum.get(), &outputViewDesc,
...
bool WgcNvencGpuProbe::Phase15_CaptureConvertMapEncodeLoop() {
    ...
    while (true) {
        ...
        HRESULT hr = m_videoDevice->CreateVideoProcessorInputView(latestTex.get(), m_videoEnum.get(), &inputViewDesc,
                                                                  inputView.put());
        if (FAILED(hr) || inputView == nullptr) {
            char buf[80];
            snprintf(buf, sizeof(buf), "frame %d: CreateVideoProcessorInputView failed 0x%08lX", frameIdx,
        ...
        hr = m_videoContext->VideoProcessorBlt(m_videoProcessor.get(), m_videoOutputView.get(),
                                               0, // OutputFrame index
                                               1, // NumStreams
                                               &stream);
        ...
        if (FAILED(hr)) {
            char buf[80];
            snprintf(buf, sizeof(buf), "frame %d: VideoProcessorBlt failed 0x%08lX", frameIdx,
                     static_cast<unsigned long>(hr));
            PrintPhaseFail(15, "capture-convert-map-encode loop", buf);
            return false;
        }
        ...
        if (nvSt != NV_ENC_SUCCESS) {
            char buf[96];
            snprintf(buf, sizeof(buf), "frame %d: nvEncMapInputResource: %s", frameIdx, NvencStatusName(nvSt));
            PrintPhaseFail(15, "capture-convert-map-encode loop", buf);
            return false;
        }
...
bool WgcNvencGpuProbe::Phase19_VerifyIvfFile() {
    ...
    if (!GetFileAttributesExW(L"probe_wgc_nvenc_gpu_output\\wgc_gpu_av1.ivf", GetFileExInfoStandard, &data)) {
        PrintPhaseFail(19, "verify IVF file", "GetFileAttributesExW failed — file not found or inaccessible");
        return false;
    }
...
int WgcNvencGpuProbe::Run() {
    fprintf(stdout, "[probe] === WGC -> GPU NV12 -> NVENC AV1 GPU-path integration probe ===\n");
    auto kindStr = (m_target.kind == CaptureTarget::Kind::Monitor) ? L"monitor" : L"window";
    ...
    PrintSummary();
    fprintf(stdout, "[probe] encode probe PASS — WGC -> GPU NV12 -> NVENC AV1 IVF GPU-path validated.\n");
    fflush(stdout);
    return 0;
}

...
TEST_F(InstallTreeTest, AllExpectedLicensesPresent) {
    SKIP_IF_NO_TREE();
    // Every dependency staged unconditionally for the default build configuration
    // (third_party/CMakeLists.txt + cmake/VendorFFmpeg.cmake). Crash-capture licenses
    // (sentry-native/crashpad/mini_chromium) are official-release-build-only

...
**Proposed — awaiting maintainer decision.** This document lays out the problem and the
available options; it does not choose one. No code changes accompany it.
...
None yet — no code changes accompany this document. Whichever option is chosen will need its own
follow-up ADR-amendment or superseding ADR recording the actual decision, the build changes made,
and the updated `THIRD_PARTY_NOTICES.md`/CMake comment text.

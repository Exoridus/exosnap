...
- **Format & encoding:** container selection is now the compact segmented control (`containerSegmented` with MKV/WebM/MP4 buttons). Video/audio codec reconciliation is capability-aware, frame rate is selectable at 24/25/30/50/60 fps, 120 fps is shown unavailable until active hardware/source support is proven, and CFR/VFR timing is real. MP4 is reconciled to CFR because the current Media Foundation mux path is fixed-rate.
- **Output:** output resolution is now functional for Native, 4K, 1440p, 1080p, and 720p. The recorder resolves exact even output dimensions, uses contain-fit letterboxing, and scales on the D3D11 GPU path into the encoder input. Custom resolution fields exist in the preset schema but are not exposed in Settings in this slice.
- **Preset/schema:** complete recording presets now persist output resolution intent, fit mode, frame rate, timing mode, container, codecs, quality, and cursor state under schema version 2.
- **Visual Harness:** deterministic output/format scenarios now report requested/effective dimensions, source dimensions, content rectangle, timing, codecs, reconciliation warning, lock state, and preset dirty state.
...
- **Hotkeys:** `HotkeysPage` reflowed into compact table-like rows inside two `panel` frames, `ACTIVE HOTKEYS` (Available now) and `PLANNED / UNAVAILABLE` (Not in this build), with hairline row separators and no oversized cards.

  Bindings render via the new `ui/widgets/KeycapChip`: IBM-Plex-Mono keycaps with a subtle border and bottom-edge shadow, joined by dim `+` labels, and a muted `Unset` chip when empty.

  Active rows keep the genuinely supported Set and Unset rebind (`QKeySequenceEdit`) plus a new `Reset to defaults` button. Planned rows expose **no** rebind or keycap controls, only an honest `Not in this build` tag. A footnote states that shortcuts register globally and that ExoSnap does **not** detect conflicts.

  Only the four backend-modelled actions are shown. Start/Stop and Pause/Resume are the real WM_HOTKEY-wired ones; Split and Mute Mic are presented as planned, registered but with no live handler. No new global hotkey backend, no rebinding where unsupported, and no fake conflict detection was added.
...
- **Inline About (Part A):** the separate native About `QDialog` is removed. About is now an in-window overlay (`ui/dialogs/AboutOverlay`), a translucent backdrop painted in code with a centered `#aboutCard`.

  It is parented to the page **stack**, so the title bar and window controls stay usable and correctly painted, with no semi-transparent sibling shine-through on the chrome. Qt manages native-window z-order when the alien overlay is `raise()`d above the native `PreviewSurface`, so it renders cleanly over the Record page: the DXGI preview yields behind the card, and `PreviewSurface` and DXGI are untouched.

  The top-nav About action opens it. Close, `Escape`, a backdrop click or any nav switch dismiss it, and `navigateToPage()` closes it so it never lingers over an unrelated page. No native title bar, and no focus-sticky child window after closing.

  The R6 card content and actions are retained verbatim: aperture mark, two-tone wordmark, Version line, description, the real VERSION, BUILD, COMMIT and AUTHOR table, `GitHub` as a configured URL only, `Copy details`, and `Close`.

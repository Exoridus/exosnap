...
TEST(HdrColorSpace, BothPqRangesCountAsHdr) {
    EXPECT_TRUE(exosnap::engine::IsHdrColorSpace(DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020));
    EXPECT_TRUE(exosnap::engine::IsHdrColorSpace(DXGI_COLOR_SPACE_RGB_STUDIO_G2084_NONE_P2020));
}

TEST(HdrColorSpace, NonPqColorSpacesAreNotHdr) {
    // sRGB desktop.
    EXPECT_FALSE(exosnap::engine::IsHdrColorSpace(DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P709));
    // scRGB FP16 — an Advanced-Color SDR desktop, not HDR (see OdCaptureMode::SdrScrgb).
    EXPECT_FALSE(exosnap::engine::IsHdrColorSpace(DXGI_COLOR_SPACE_RGB_FULL_G10_NONE_P709));
}

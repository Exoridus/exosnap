...
using exosnap::engine::ColorRange;
using exosnap::engine::ConvertAyuvToBgra;
using exosnap::engine::ConvertFullPlanar444ToBgra;
using exosnap::engine::ConvertFullPlanarYuv420ToBgra;
using exosnap::engine::ConvertYuv420ToBgra;
using exosnap::engine::FullPlanar444Frame;
using exosnap::engine::FullPlanarYuv420Frame;
using exosnap::engine::MatrixCoefficients;
using exosnap::engine::PackedAyuvFrame;
using exosnap::engine::PlanarYuv420Frame;
using exosnap::engine::YuvToBgraParams;
...
TEST(FullPlanar444ToBgraSimd, MatchesTheScalarReferenceByteForByte) {
    if (!exosnap::engine::CpuSupportsYuvToBgraSimd())
        ...
        for (const uint32_t height : {2u, 5u}) {
            for (const auto matrix :
                ...
                for (const auto range : {ColorRange::Limited, ColorRange::Full}) {
                    ...
                    exosnap::engine::ConvertFullPlanar444ToBgraScalar(f.src, params, expected.data(), stride);
                    exosnap::engine::ConvertFullPlanar444ToBgraSimd(f.src, params, actual.data(), stride);
...
TEST(FullPlanarYuv420ToBgraSimd, MatchesTheScalarReferenceByteForByte) {
    ...
    for (const uint32_t width : {8u, 16u, 24u, 9u, 11u, 15u, 17u, 2u, 6u}) {
        for (const uint32_t height : {2u, 5u}) {
            for (const auto matrix :
                 {exosnap::engine::MatrixCoefficients::Bt709, exosnap::engine::MatrixCoefficients::Bt601,
                  exosnap::engine::MatrixCoefficients::Bt2020Ncl}) {
                for (const auto range : {exosnap::engine::ColorRange::Limited, exosnap::engine::ColorRange::Full}) {
                    ...
                    std::vector<uint8_t> expected(static_cast<size_t>(stride) * height, 0u);
                    std::vector<uint8_t> actual(expected.size(), 0u);
                    exosnap::engine::ConvertFullPlanarYuv420ToBgraScalar(f.src, params, expected.data(), stride);
                    exosnap::engine::ConvertFullPlanarYuv420ToBgraSimd(f.src, params, actual.data(), stride);
...
TEST(FullPlanarYuv420ToBgraSimd, TenBitInputGoesThroughTheScalarReference) {
    ...
    std::vector<uint8_t> actual(expected.size(), 0u);
    exosnap::engine::ConvertFullPlanarYuv420ToBgraScalar(src, params, expected.data(), stride);
    exosnap::engine::ConvertFullPlanarYuv420ToBgraSimd(src, params, actual.data(), stride);

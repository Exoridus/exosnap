...
TEST(QualityPresetMapping, CanonicalCqMatchesTheFiveTierLadder) {
    EXPECT_EQ(exosnap::engine::CanonicalCq(exosnap::engine::QualityPreset::Ultra), 16u);
    EXPECT_EQ(exosnap::engine::CanonicalCq(exosnap::engine::QualityPreset::High), 19u);
    EXPECT_EQ(exosnap::engine::CanonicalCq(exosnap::engine::QualityPreset::Balanced), 24u);
    EXPECT_EQ(exosnap::engine::CanonicalCq(exosnap::engine::QualityPreset::Low), 30u);
    EXPECT_EQ(exosnap::engine::CanonicalCq(exosnap::engine::QualityPreset::Draft), 35u);
}

TEST(QualityPresetMapping, NearestPresetRoundTripsAndSnapsBetweenValues) {
    using exosnap::engine::NearestQualityPreset;
    using exosnap::engine::QualityPreset;
...
TEST(QualityPresetMapping, IsCanonicalCqOnlyForTheFiveNamedValues) {
    for (uint32_t cq = exosnap::engine::kCqMin; cq <= exosnap::engine::kCqMax; ++cq) {

...
using exosnap::engine::RgbToAyuvConverter;
...
std::vector<Rgb8> RoundTrip(D3DTestDevice& d3d, const std::vector<Rgb8>& pixels, bool full_range) {
    ...
    exosnap::engine::PackedAyuvFrame frame;
    ...
    params.range = full_range ? exosnap::engine::ColorRange::Full : exosnap::engine::ColorRange::Limited;

    std::vector<uint8_t> bgra(static_cast<size_t>(width) * 4u, 0);
    exosnap::engine::ConvertAyuvToBgra(frame, params, bgra.data(), static_cast<uint32_t>(width) * 4u);

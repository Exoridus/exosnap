...
//
// Every other engine test either drives a single component in isolation
...
//           runner, so exosnap_engine_testutil's SyntheticSession plays
...
bool RunRealPipelineToMkv(VideoCodec video_codec, AudioCodec audio_codec, const std::string& mkv_path,
    ...
    const auto r = exosnap::engine::testutil::SyntheticSession(cfg).Run();
    if (!r.success)
        out_error = r.error;
    return r.success;
}
...
TEST_F(SessionE2ETest, RealPipeline_H264Aac_ProducesPlayableMkvAndMp4) {
    ...
    const auto rr = exosnap::engine::RemuxToProgressiveMp4(mkv, mp4);

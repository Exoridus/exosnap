...
TEST(OdCaptureFormatPolicyTest, SupportedFormats) {
    EXPECT_TRUE(exosnap::engine::IsSupportedOdCaptureFormat(DXGI_FORMAT_B8G8R8A8_UNORM));
    EXPECT_TRUE(exosnap::engine::IsSupportedOdCaptureFormat(DXGI_FORMAT_R10G10B10A2_UNORM));
    ...
    EXPECT_TRUE(exosnap::engine::IsSupportedOdCaptureFormat(DXGI_FORMAT_R16G16B16A16_FLOAT));
    EXPECT_FALSE(exosnap::engine::IsSupportedOdCaptureFormat(DXGI_FORMAT_R8G8B8A8_UNORM));
    EXPECT_FALSE(exosnap::engine::IsSupportedOdCaptureFormat(DXGI_FORMAT_NV12));
    EXPECT_FALSE(exosnap::engine::IsSupportedOdCaptureFormat(DXGI_FORMAT_UNKNOWN));
}

TEST(OdCaptureFormatPolicyTest, FormatNames) {
    char buf[32];
    EXPECT_STREQ("B8G8R8A8_UNORM (8-bit SDR)",
                 exosnap::engine::OdCaptureFormatName(DXGI_FORMAT_B8G8R8A8_UNORM, buf, sizeof(buf)));
    EXPECT_STREQ("R10G10B10A2_UNORM (10 bpc SDR)",
                 exosnap::engine::OdCaptureFormatName(DXGI_FORMAT_R10G10B10A2_UNORM, buf, sizeof(buf)));
    EXPECT_STREQ("R16G16B16A16_FLOAT (HDR/Advanced Color)",
                 exosnap::engine::OdCaptureFormatName(DXGI_FORMAT_R16G16B16A16_FLOAT, buf, sizeof(buf)));
    // Unknown formats fall back to a numeric rendering (never crashes/nullptr).
    EXPECT_STREQ("DXGI_FORMAT(28)", exosnap::engine::OdCaptureFormatName(DXGI_FORMAT_R8G8B8A8_UNORM, buf, sizeof(buf)));
}
...
TEST(SessionStateWebcamOverlayLiveTest, SeedUpdateAndSnapshotSanitizeLiveOverlay) {
    ...
    EXPECT_FLOAT_EQ(updated.overlay_h_norm, exosnap::engine::WebcamPlacement::kMinSize);

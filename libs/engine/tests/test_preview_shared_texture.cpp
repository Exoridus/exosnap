...
using exosnap::engine::kPreviewSharedConsumerKey;
using exosnap::engine::kPreviewSharedProducerKey;
using exosnap::engine::PreviewSharedTexture;
...
TEST(PreviewSharedTexture, PublishDropsWhileConsumerHoldsMutex) {
    ...
    const exosnap::engine::PreviewSharedTexture::PublishResult contended =
        shared.TryPublish(producer.context.get(), src.get());
    EXPECT_FALSE(contended.published());
    EXPECT_EQ(contended.acquire, exosnap::engine::PreviewAcquireOutcome::Contended);

    // Once the consumer releases, publishing succeeds again.
    consumerMutex->ReleaseSync(kPreviewSharedProducerKey);
    const exosnap::engine::PreviewSharedTexture::PublishResult resumed =
        shared.TryPublish(producer.context.get(), src.get());
    EXPECT_TRUE(resumed.published());
    EXPECT_EQ(resumed.acquire, exosnap::engine::PreviewAcquireOutcome::Acquired);

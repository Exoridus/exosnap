...
TEST(AudioPlanValidateTest, Validate_AcceptsPcmFloatWithBitDepth32) {
    RecorderSession session;
    RecorderConfig cfg = MakeValidBaseConfig();
    cfg.container = exosnap::engine::Container::Matroska; // Pcm is MKV-only
    cfg.audio_codec = exosnap::engine::AudioCodec::Pcm;
...
TEST(AudioPlanValidateTest, Validate_RejectsPcmFloatWithNonPcmCodec_Aac) {
    RecorderSession session;
    RecorderConfig cfg = MakeValidBaseConfig();
    cfg.container = exosnap::engine::Container::Matroska; // valid for Aac, isolates the pcm_float check
...
TEST(AudioPlanValidateTest, Validate_RejectsPcmFloatWithNonPcmCodec_Opus) {
    RecorderSession session;
    RecorderConfig cfg = MakeValidBaseConfig();
    cfg.audio_codec = exosnap::engine::AudioCodec::Opus; // Opus is valid on the default WebM container
...
TEST(AudioPlanValidateTest, Validate_RejectsPcmFloatWithNonPcmCodec_Flac) {
    RecorderSession session;
    RecorderConfig cfg = MakeValidBaseConfig();
    cfg.container = exosnap::engine::Container::Matroska; // valid for Flac, isolates the pcm_float check
    cfg.audio_codec = exosnap::engine::AudioCodec::Flac;

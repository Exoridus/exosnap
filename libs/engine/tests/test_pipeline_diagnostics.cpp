...
TEST(PipelineDiagnostics, PerfSessionSummaryHistogramsAndReset) {
    ...
    const std::size_t b5 = exosnap::engine::LatencyHistogram::BucketIndex(5.0);
    EXPECT_EQ(sum.encode_latency.buckets[b5], 30u);
    EXPECT_GE(sum.encode_latency.p50_ms, exosnap::engine::LatencyHistogram::BucketLowEdge(b5));
    EXPECT_LE(sum.encode_latency.p50_ms, exosnap::engine::LatencyHistogram::BucketHighEdge(b5));
...
TEST(GpuStageTiming, WholeSessionHistogramsAccumulateAndReset) {
    ...
    const std::size_t b3 = exosnap::engine::LatencyHistogram::BucketIndex(3.0);

...
using exosnap::engine::DecodedPixelFormat;
using exosnap::engine::EditFrameGpuConverter;
using exosnap::engine::MatrixCoefficients;
using exosnap::engine::RawDecodedVideoFrame;
...
std::vector<uint8_t> Cpu420(const Planes8& p, MatrixCoefficients matrix, ColorRange range) {
    ...
    exosnap::engine::YuvToBgraParams params{matrix, range};
    std::vector<uint8_t> out(static_cast<size_t>(kWidth) * kHeight * 4u);
    exosnap::engine::ConvertFullPlanarYuv420ToBgraScalar(src, params, out.data(), kWidth * 4u);
    return out;
}
...
std::vector<uint8_t> Cpu444(const Planes8& p, MatrixCoefficients matrix, ColorRange range) {
    ...
    exosnap::engine::YuvToBgraParams params{matrix, range};
    std::vector<uint8_t> out(static_cast<size_t>(kWidth) * kHeight * 4u);
    exosnap::engine::ConvertFullPlanar444ToBgraScalar(src, params, out.data(), kWidth * 4u);
    return out;
}
...
std::vector<uint8_t> CpuPqExact(const Planes10& p, float peak_scale) {
    ...
    for (uint32_t row = 0; row < kHeight; ++row) {
        for (uint32_t col = 0; col < kWidth; ++col) {
            ...
            const exosnap::engine::MonitorBgr bgr =
                exosnap::engine::P010PqPixelToMonitorBgr(y10, cb10, cr10, peak_scale);
...
TEST(EditFrameGpuConverterTest, PqSourceAgreesWithShippingCpuConverterWithinTableError) {
    ...
    exosnap::engine::P010PqMonitorConverter(kPeak1000).Convert(src, lut.data(), kWidth * 4u);

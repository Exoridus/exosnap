...
// control/protocol.h -- the wire format of an ExoSnap control channel.
//
// Shared by every process that exposes one: the application's Live Verify
// endpoint and the updater's automation endpoint speak the same envelope, the
// same error taxonomy and the same settle semantics. That is not tidiness -- a
// second, separately-written protocol is a second set of edge cases, and the
// runner that drives both would have to know which is which.
//
// Nothing here knows what a command IS. The command table, the state and the
// preconditions belong to the process; this file is the envelope around them.
//

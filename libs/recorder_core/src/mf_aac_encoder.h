...
class MfAacEncoder {
  public:
    ...
    IMFMediaType* OutputMediaType() const {
        return m_pOutputType;
    }
    ...
    void FeedFloat32(const float* float_data, size_t total_float_samples, uint64_t pts_ns, uint64_t& accumulated_frames,
                     uint32_t sample_rate, uint32_t channels, std::vector<EncodedAudioPacket>& out_packets);

    // Feed raw PCM (when the encoder accepts PCM directly).
    void FeedRaw(const BYTE* data, DWORD data_bytes, uint64_t pts_ns, LONGLONG duration_100ns,
    ...
    bool UsesPcm16() const {
        return m_inputSubtype == MFAudioFormat_PCM && m_inputBitsPerSample == 16;
    }
    GUID InputSubtype() const {
        return m_inputSubtype;
    }
    UINT32 InputBps() const {
        return m_inputBitsPerSample;
    }
    UINT32 InputBlockAlign() const {
        return m_inputBlockAlign;
    }

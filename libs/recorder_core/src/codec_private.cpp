...
#define CHECK_OVERFLOW(ctx) \
    ...
    for (uint32_t i = 0; i <= operating_points_cnt_minus_1; ++i) {
        br.f(12); CHECK_OVERFLOW("operating_point_idc");
    ...
    if (mono_chrome) {
        br.f(1); CHECK_OVERFLOW("color_range(mono)");
        subsampling_x          = 1;
        subsampling_y          = 1;
        ...
        br.f(1); CHECK_OVERFLOW("color_range");
        if (seq_profile == 0) {
            subsampling_x = 1;
            subsampling_y = 1;
        ...
        br.f(1); CHECK_OVERFLOW("separate_uv_delta_q");
    }
    ...
    size_t         reason_buf_size)
{
        ...
        if (obu_type == 1) {
            if (i + payload_size > bsLen) {
                snprintf(reason_buf, reason_buf_size,
                         "Sequence Header OBU payload exceeds packet boundary");
                return false;
            }
            obu_payload      = bs + i;
    ...
    if (obu_payload == nullptr) {
        snprintf(reason_buf, reason_buf_size,
                 "Sequence Header OBU (type 1) not found in bitstream packet");
        return false;
    }
    ...
    out_av1_codec_private[2] = static_cast<uint8_t>(
        (seq_tier_0             << 7) |
        (high_bitdepth          << 6) |
        (twelve_bit             << 5) |
        (mono_chrome            << 4) |
    ...
    size_t        reason_buf_size)
{
    ...
    if (FAILED(hr)) {
        snprintf(reason_buf, reason_buf_size,
                 "GetBlob(MF_MT_USER_DATA) failed 0x%08lX",

...
HRESULT BuildAudioMediaType(const GUID& subtype, UINT32 sample_rate, UINT32 channels, UINT32 bits_per_sample,
...
bool MfAacEncoder::Init(uint32_t sample_rate, uint32_t channels, std::string& out_error) {
    ...
    UINT32 count = 0;
    hr = MFTEnumEx(MFT_CATEGORY_AUDIO_ENCODER, kEnumFlags, &inType, &outType, &ppActivate, &count);
    if (FAILED(hr)) {
        char buf[80];
...
void MfAacEncoder::FeedFloat32(const float* float_data, size_t total_float_samples, uint64_t pts_ns,
    ...
    LONGLONG sampleDuration = static_cast<LONGLONG>(numFrames) * 10000000LL / static_cast<LONGLONG>(sample_rate);

    UINT32 dataBytes = numFrames * m_inputBlockAlign;

    IMFMediaBuffer* pMFBuf = nullptr;
    if (FAILED(MFCreateMemoryBuffer(dataBytes, &pMFBuf)))
        return;
    ...
    if (UsesPcm16()) {
        ConvertFloat32ToPcm16(float_data, reinterpret_cast<int16_t*>(pDst), static_cast<size_t>(numFrames) * channels);
    } else {
...
void MfAacEncoder::FeedRaw(const BYTE* data, DWORD data_bytes, uint64_t pts_ns, LONGLONG duration_100ns,
                           std::vector<EncodedAudioPacket>& out_packets) {
    IMFMediaBuffer* pMFBuf = nullptr;
    if (FAILED(MFCreateMemoryBuffer(data_bytes, &pMFBuf)))
        return;

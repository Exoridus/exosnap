...
const QVector<VisualScenario> kScenarios = {
    ...
    {QStringLiteral("record-preparing"),
     ...
     0,
     0},
    {.id = QStringLiteral("record-output-unwritable"),
     .title = QStringLiteral("Record / Output Unwritable"),
     .page = VisualPage::Record,
     .record_state = VisualRecordState::Ready,
     .masks = {{QStringLiteral("previewSurface"), QStringLiteral("live preview remains dynamic")}},
     .record_visual_state = QString::fromLatin1(record_state::kOutputUnwritable)},
    {.id = QStringLiteral("settings-output-unwritable"),
     .title = QStringLiteral("Settings / Output Unwritable"),
     .page = VisualPage::Settings,
     .record_visual_state = QString::fromLatin1(record_state::kOutputUnwritable)},
    {.id = QStringLiteral("record-output-preparing"),
     .title = QStringLiteral("Record / Output Preparing"),
     .page = VisualPage::Record,
     .record_state = VisualRecordState::Preparing,
     .masks = {{QStringLiteral("previewSurface"), QStringLiteral("live preview remains dynamic")},
               {QStringLiteral("recordTransportDock"), QStringLiteral("meter values are deterministic test levels")}},
     .record_visual_state = QString::fromLatin1(record_state::kPreparing)},
    ...
    {.id = QStringLiteral("record-webcam-error"),
     .title = QStringLiteral("Record / Webcam Open Failure"),
     .page = VisualPage::Record,
     .record_state = VisualRecordState::Ready,
     ...
     .webcam_w = 0.25f,
     .webcam_h = 0.25f,
     .webcam_open_failed = true,
     .webcam_open_fail_reason = QStringLiteral("Device in use by another app")},
    ...
    {.id = QStringLiteral("diagnostics-idle"),
     ...
     .diag_live = QStringLiteral("idle")},
    {.id = QStringLiteral("diagnostics-recording-healthy"),
     .title = QStringLiteral("Diagnostics — recording (healthy)"),
     .page = VisualPage::Diagnostics,
     .diag_live = QStringLiteral("healthy")},
    {.id = QStringLiteral("diagnostics-recording-encoder-pressure"),
     .title = QStringLiteral("Diagnostics — encoder pressure"),
     .page = VisualPage::Diagnostics,
     .diag_live = QStringLiteral("encoder")},
    {.id = QStringLiteral("diagnostics-recording-disk-pressure"),
    ...
    {.id = QStringLiteral("diagnostics-paused"),
     ...
     .page = VisualPage::Diagnostics,
     .diag_live = QStringLiteral("paused")},
    {.id = QStringLiteral("diagnostics-split-pending"),
     .title = QStringLiteral("Diagnostics — split pending"),
     .page = VisualPage::Diagnostics,
     .diag_live = QStringLiteral("split")},
    // ── Honesty-rail lifecycle states (4-tier model) ────────────────────────────
    // Tier-1 blocker: MP4 + FLAC is a hard container incompatibility (rec.009).
    {.id = QStringLiteral("diagnostics-blocked"),
     .title = QStringLiteral("Diagnostics — blocked (Tier-1)"),
     .page = VisualPage::Diagnostics,
    ...
    {.id = QStringLiteral("diagnostics-issues"),
     ...
     .page = VisualPage::Diagnostics,
     .diag_live = QStringLiteral("judder")},
    // Tier-2 measured problem: audio device loss mid-recording (rec.audio.degraded).
    {.id = QStringLiteral("diagnostics-audio-degraded"),
     .title = QStringLiteral("Diagnostics — audio device lost (Tier-2)"),
     .page = VisualPage::Diagnostics,
...
const QVector<VisualScenario> kEditExportScenarios = {
    {.id = QStringLiteral("edit-main"),
     .title = QStringLiteral("Edit Export / Main view"),
     ...
     .edit_export_playhead_ms = 77000,
     .edit_export_thumbnail_tiles = -1},
    {.id = QStringLiteral("edit-trimmed"),
     .title = QStringLiteral("Edit Export / Trimmed (playhead mid-clip)"),
     .page = VisualPage::EditExport,
     .edit_export_file_path = QStringLiteral("C:\\Users\\User\\Videos\\ExoSnap\\2026-06-19_Sprint-demo.mkv"),
     .edit_export_duration = QStringLiteral("00:04:18"),
     .edit_export_size = QStringLiteral("612 MB"),
     ...
     .edit_export_duration_seconds = 258.0, // 00:04:18
     .edit_export_marker_times_ms = {15000, 90000, 129000, 220000},
     .edit_export_trim_start_ms = 31000, // in 00:31
     .edit_export_trim_end_ms = 227000,  // out 03:47
     .edit_export_playhead_ms = 77000,
     .edit_export_thumbnail_tiles = -1},
    // Two named audio rows under the video row: the stack a recording with
    // separate system and microphone tracks opens with.
    {.id = QStringLiteral("edit-timeline-multitrack"),
     .title = QStringLiteral("Edit Export / Timeline / Two audio tracks"),
     .page = VisualPage::EditExport,
     .edit_export_file_path = QStringLiteral("C:\\Users\\User\\Videos\\ExoSnap\\2026-06-19_Sprint-demo.mkv"),
     .edit_export_duration = QStringLiteral("00:04:18"),
     .edit_export_size = QStringLiteral("612 MB"),
     ...
     .edit_export_audio_codec = QStringLiteral("Opus"),
     .edit_export_container = QStringLiteral("MKV"),
     .edit_export_duration_seconds = 258.0,
     .edit_export_marker_times_ms = {15000, 90000, 129000, 220000},
     .edit_export_playhead_ms = 77000,
     .edit_export_audio_track_labels = {QStringLiteral("System"), QStringLiteral("Microphone")},
     .edit_export_thumbnail_tiles = -1},
    // The strip mid-decode: the tiles that have arrived are drawn, the rest of
    // the row is simply empty -- no spinner, no placeholder pattern.
    {.id = QStringLiteral("edit-timeline-loading"),
     .title = QStringLiteral("Edit Export / Timeline / Thumbnails still arriving"),
     .page = VisualPage::EditExport,
     .edit_export_file_path = QStringLiteral("C:\\Users\\User\\Videos\\ExoSnap\\2026-06-19_Sprint-demo.mkv"),
     .edit_export_duration = QStringLiteral("00:04:18"),
     .edit_export_size = QStringLiteral("612 MB"),
     ...
     .edit_export_marker_times_ms = {15000, 90000, 129000, 220000},
     .edit_export_playhead_ms = 77000,
     .edit_export_thumbnail_tiles = 4},
    {.id = QStringLiteral("edit-export-options"),
     .title = QStringLiteral("Edit Export / Export panel / Options"),
     .page = VisualPage::EditExport,
     .edit_export_panel_state = QStringLiteral("options"),
     .edit_export_file_path = QStringLiteral("C:\\Users\\User\\Videos\\ExoSnap\\2026-06-19_Sprint-demo.mkv"),
     .edit_export_duration = QStringLiteral("00:04:18"),
     .edit_export_size = QStringLiteral("612 MB"),
     .edit_export_resolution = QStringLiteral("2560 \xc3\x97 1440"),
     ...
     .edit_export_container = QStringLiteral("MKV"),
     .edit_export_duration_seconds = 258.0},
    {.id = QStringLiteral("edit-export-running"),
     .title = QStringLiteral("Edit Export / Export panel / Running"),
     .page = VisualPage::EditExport,
     .edit_export_panel_state = QStringLiteral("running"),
     .edit_export_file_path = QStringLiteral("C:\\Users\\User\\Videos\\ExoSnap\\2026-06-19_Sprint-demo.mkv"),
     .edit_export_duration = QStringLiteral("00:04:18"),
     .edit_export_size = QStringLiteral("612 MB"),
     .edit_export_resolution = QStringLiteral("2560 \xc3\x97 1440"),
     ...
     .edit_export_container = QStringLiteral("MKV"),
     .edit_export_duration_seconds = 258.0},
    {.id = QStringLiteral("edit-export-done"),
     .title = QStringLiteral("Edit Export / Export panel / Done"),
     .page = VisualPage::EditExport,
     .edit_export_panel_state = QStringLiteral("done"),
     .edit_export_file_path = QStringLiteral("C:\\Users\\User\\Videos\\ExoSnap\\2026-06-19_Sprint-demo.mkv"),
     .edit_export_duration = QStringLiteral("00:04:18"),
     .edit_export_size = QStringLiteral("612 MB"),
     .edit_export_resolution = QStringLiteral("2560 \xc3\x97 1440"),
     ...
     .edit_export_container = QStringLiteral("MKV"),
     .edit_export_duration_seconds = 258.0},
    {.id = QStringLiteral("edit-export-failed"),
     .title = QStringLiteral("Edit Export / Export panel / Failed"),
     .page = VisualPage::EditExport,
     .edit_export_panel_state = QStringLiteral("failed"),
     .edit_export_file_path = QStringLiteral("C:\\Users\\User\\Videos\\ExoSnap\\2026-06-19_Sprint-demo.mkv"),
     .edit_export_duration = QStringLiteral("00:04:18"),
     .edit_export_size = QStringLiteral("612 MB"),
     .edit_export_resolution = QStringLiteral("2560 \xc3\x97 1440"),
     ...
     .edit_export_container = QStringLiteral("MKV"),
     .edit_export_duration_seconds = 258.0},
    {.id = QStringLiteral("edit-report-warning"),
     .title = QStringLiteral("Edit Export / Post-flight report warning"),
     .page = VisualPage::EditExport,
     .edit_export_report_severity = QStringLiteral("warning"),
     .edit_export_file_path = QStringLiteral("C:\\Users\\User\\Videos\\ExoSnap\\2026-06-19_Sprint-demo.mkv"),
     .edit_export_duration = QStringLiteral("00:04:18"),
     .edit_export_size = QStringLiteral("612 MB"),
     .edit_export_resolution = QStringLiteral("2560 \xc3\x97 1440"),
...
const QVector<VisualScenario> kEditPlayerSurfaceScenarios = {
    ...
    {.id = QStringLiteral("edit-player-surface-empty"),
     .title = QStringLiteral("Edit Export / Player Surface Empty (isolation)"),
     .page = VisualPage::EditExport,
    ...
    {.id = QStringLiteral("edit-player-surface-frame"),
     .title = QStringLiteral("Edit Export / Player Surface Frame (isolation)"),
     .page = VisualPage::EditExport,

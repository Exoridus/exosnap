...
class RecordViewModel {
  public:
    ...
    uint64_t output_file_bytes = 0;
    double elapsed_seconds = 0.0;
    // REAL frame drops only (encoder backpressure or a frame-processing failure) --
    // never deliberate CFR pacing/coalescing (e.g. downsampling a 144 Hz source to
    // 60 fps target, which is normal and not a drop). Sourced from the diagnostics
    // snapshot's capture.frames_dropped_problem(), not SessionStats -- see the
    // comment by av_drift_ms below and RecordPage's diagnostics callback.

...
TEST(AudioEncodingPreset, StoreRoundTrip_AudioCodec_Flac) {
    // 0.6.0 Audio v2: FLAC (MKV-only) must round-trip through save/load. The
    // default preset is MKV + AV1, so MKV + AV1 + FLAC is an Allowed combo and
    // survives SanitizePresetConfig's container/codec reconciliation.
    const QString path = UniqueTempPath();
    RecordingPresetStore store(path);

...
QWidget* makeTransparencyColumn(const QString& heading, const char* color_base, const QStringList& items,
    ...
    for (const QString& item : items) {
        ...
        text->setStyleSheet(
            QStringLiteral("font-size:11px; color:%1; background:transparent;").arg(tok(ExoSnapPalette::kText2)));
    ...
    if (left_rule)
        col->setStyleSheet(QStringLiteral("QWidget { border-left:1px solid %1; }").arg(tok(ExoSnapPalette::kLine1)));
    return col;
}
...
CrashReportPanel::CrashReportPanel(const CrashReportModel& model, QWidget* parent) : QWidget(parent), model_(model) {
    ...
    setStyleSheet(QStringLiteral("#crashReportCard { background:%1; border:1px solid %2; border-radius:14px; }")
                      .arg(tok(ExoSnapPalette::kBg1), tok(ExoSnapPalette::kLine2)));
    ...
    auto_send_check_->setStyleSheet(
        QStringLiteral("QCheckBox { font-size:12.5px; color:%1; spacing:10px; background:transparent; }"
            ...
            .arg(tok(ExoSnapPalette::kText2), tok(ExoSnapPalette::kLine2), tok(ExoSnapPalette::kAccent)));
...
QWidget* CrashReportPanel::buildChromeBar() {
    ...
    bar->setStyleSheet(QStringLiteral("#crashChromeBar { background:%1; border-bottom:1px solid %2; "
                           ...
                           .arg(tok(ExoSnapPalette::kBg0), tok(ExoSnapPalette::kLine1)));
    ...
    sep->setStyleSheet(QStringLiteral("background:%1; border:none;").arg(tok(ExoSnapPalette::kLine2)));
...
QWidget* CrashReportPanel::buildRecordingBanner() {
    ...
    banner->setStyleSheet(
        QStringLiteral("#crashRecordingBanner { background:%1; border:1px solid %2; border-radius:10px; }")
            .arg(rgba(ExoSnapPalette::kOk, 0.13), rgba(ExoSnapPalette::kOk, 0.45)));
...
QWidget* CrashReportPanel::buildTransparencyBlock() {
    ...
    note->setStyleSheet(
        QStringLiteral("font-size:11px; color:%1; background:transparent;").arg(tok(ExoSnapPalette::kText3)));
...
QWidget* CrashReportPanel::buildScrubbedReport() {
    ...
    stack_label->setStyleSheet(
        QStringLiteral("font-family:'IBM Plex Mono','Consolas',monospace; font-size:10.5px; color:%1; "
            ...
            .arg(tok(ExoSnapPalette::kText2), tok(ExoSnapPalette::kLine2)));
    ...
    footer->setStyleSheet(QStringLiteral("border-top:1px solid %1;").arg(tok(ExoSnapPalette::kLine1)));

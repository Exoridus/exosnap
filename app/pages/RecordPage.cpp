...
void RecordPage::setWebcamSettings(const WebcamSettings& settings) {
    ...
    if (preview_surface_) {
        ...
        preview_surface_->setWebcamChromaKey(s.chroma_key);
...
void RecordPage::updateWebcamOverlay() {
    ...
    preview_surface_->setWebcamChromaKey(current_webcam_settings_.chroma_key);
...
void RecordPage::applyVisualScenario(const visual::VisualScenario& scenario) {
    ...
    case visual::VisualRecordState::Completed: {
        ...
        case capability::AudioCodec::Flac:
            result.audio_codec = recorder_core::AudioCodec::Flac;
            break;
        }
...
void RecordPage::onHotkeyCaptureFrame() {
    ...
    if (st == UiRecordingState::Ready || st == UiRecordingState::Recording || st == UiRecordingState::Paused) {
        // CAPTURE-FRAME-DOCK-BUTTON-R1: dock button is disabled via applyState() while
        // a capture is in flight; the coordinator's callback re-enables it via refresh().

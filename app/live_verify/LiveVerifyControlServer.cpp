...
LiveVerifyControlServer::LiveVerifyControlServer(LiveVerifySource* source, QString run_id, QObject* parent)
    : QObject(parent), source_(source), run_id_(std::move(run_id)), pipe_name_(PipeNameForRunId(run_id_)),
      dispatcher_(source, run_id_) {
}
...
bool LiveVerifyControlServer::Start(QString* error) {
    ...
    qInfo().noquote() << QStringLiteral("[live-verify] control endpoint ready (run %1)").arg(RedactRunId(run_id_));
    return true;
}
...
void LiveVerifyControlServer::Stop() {
    ...
    if (was_running)
        qInfo().noquote() << QStringLiteral("[live-verify] control endpoint closed");
}
...
QList<QByteArray> LiveVerifyControlServer::TakeOutbound() {
    const std::lock_guard<std::mutex> guard(outbound_mutex_);
...
void LiveVerifyControlServer::RequestSessionReset() {
    QMetaObject::invokeMethod(this, [this]() { dispatcher_.ResetSession(); }, Qt::QueuedConnection);
}

QByteArray LiveVerifyControlServer::DispatchOnGuiThread(const ParsedRequest& request, bool* close_connection) {

...
WindowEvidenceProbe::WindowEvidenceProbe(QObject* parent) : QObject(parent) {
...
WindowEvidenceProbe::Snapshot WindowEvidenceProbe::snapshot() const {
    std::lock_guard<std::mutex> lock(mutex_);
    return snapshot_;
}
...
void WindowEvidenceProbe::workerMain(std::stop_token stop_token) {
    ...
    while (!stop_token.stop_requested()) {
        ...
        if (dirty && want_hwnd != current_hwnd) {
            ...
            // Publish the cleared/started state at once.
            std::lock_guard<std::mutex> lock(mutex_);

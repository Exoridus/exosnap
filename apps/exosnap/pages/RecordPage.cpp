...
RecordPage::RecordPage(QWidget* parent) : QWidget(parent) {
    ...
    layout->addWidget(makeSubLabel(
        "Operational view for target selection, readiness, and live runtime stats.", content));

    // Capability banner
    capability_label_ = new QLabel(content);
    capability_label_->setWordWrap(true);
    capability_label_->setStyleSheet(
        "background: #1A2133; border-radius: 5px;"
    ...
    previewBox->setStyleSheet(
        "QFrame { background: #0D1017; border-radius: 6px; border: 1px solid #1E2535; }");
    ...
    target_combo_->setStyleSheet(
        "QComboBox { background: #1A2133; border: 1px solid #2A3349;"
    ...
    layout->addWidget(makeSubLabel(
        "Start is blocked automatically while active blockers are present.", content));
    ...
    start_btn_->setStyleSheet(
        "QPushButton { background: #2468C0; border: none; border-radius: 4px;"
    ...
    stop_btn_->setStyleSheet(
        "QPushButton { background: #252C3C; border: 1px solid #3A4254;"
    ...
    for (auto* lbl : {elapsed_label_, frames_label_, video_packets_label_,
                      audio_packets_label_, dropped_label_, size_label_}) {
    ...
    for (auto* lbl : {result_status_label_, result_path_label_, result_phase_label_,
                      result_hresult_label_, result_detail_label_}) {
...
void RecordPage::refresh() {
    ...
    if (view_model_.ShouldShowStats()) updateStatsDisplay();
    result_panel_->setVisible(view_model_.HasResult());
    if (view_model_.HasResult()) updateResultDisplay();
}

#pragma once

#include <windows.h>

#include <d3d11.h>
#include <d3d11_1.h>
#include <dxgi.h>

#include <winrt/Windows.Foundation.h>
#include <winrt/Windows.Graphics.Capture.h>
#include <winrt/Windows.Graphics.DirectX.Direct3D11.h>

#include <cstdint>
#include <string>
#include <vector>

#include "nvEncodeAPI.h"

namespace wgc_nvenc_gpu {

struct CaptureTarget {
    enum class Kind { Monitor, Window };

    Kind         kind        = Kind::Monitor;
    std::wstring description;
    HMONITOR     hmonitor    = nullptr;
    HWND         hwnd        = nullptr;
};

class WgcNvencGpuProbe {
  public:
    static std::vector<CaptureTarget> EnumerateTargets();

    explicit WgcNvencGpuProbe(const CaptureTarget& target);
    ~WgcNvencGpuProbe();

    WgcNvencGpuProbe(const WgcNvencGpuProbe&)            = delete;
    WgcNvencGpuProbe& operator=(const WgcNvencGpuProbe&) = delete;

    int Run();

  private:
    // Phases 01–19
    bool Phase01_InitD3D11VideoDevice();
    bool Phase02_CreateCaptureItem();
    bool Phase03_ValidateDimensions();
    bool Phase04_LoadNvencDll();
    bool Phase05_OpenNvencSession();
    bool Phase06_QueryAv1Support();
    bool Phase07_FetchPresetConfig();
    bool Phase08_InitAv1Encoder();
    bool Phase09_CreateBitstreamBuffer();
    bool Phase10_CreateNv12Texture();
    bool Phase11_ConfigureVideoProcessor();
    bool Phase12_RegisterNv12WithNvenc();
    bool Phase13_StartWgcCapture();
    bool Phase14_WaitForFirstFrame();
    bool Phase15_CaptureConvertMapEncodeLoop();
    bool Phase16_EosFlushAndDrain();
    bool Phase17_UnregisterNvencResource();
    bool Phase18_WriteIvfFile();
    bool Phase19_VerifyIvfFile();

    void CleanupCapture();
    void CleanupEncoder();
    void PrintSummary();

    // Target
    CaptureTarget m_target;
    bool          m_sourceLost = false;

    // D3D11 (shared by WGC, video processor, and NVENC)
    winrt::com_ptr<ID3D11Device>        m_d3dDevice;
    winrt::com_ptr<ID3D11DeviceContext> m_d3dContext;

    // D3D11 video interfaces (GPU BGRA->NV12)
    winrt::com_ptr<ID3D11VideoDevice>   m_videoDevice;
    winrt::com_ptr<ID3D11VideoContext>  m_videoContext;

    // Video processor
    winrt::com_ptr<ID3D11VideoProcessorEnumerator> m_videoEnum;
    winrt::com_ptr<ID3D11VideoProcessor>           m_videoProcessor;

    // NV12 intermediate texture (GPU only — no CPU access)
    winrt::com_ptr<ID3D11Texture2D>                   m_nv12Texture;
    winrt::com_ptr<ID3D11VideoProcessorOutputView>    m_videoOutputView;

    // WGC
    winrt::Windows::Graphics::Capture::GraphicsCaptureItem        m_item{nullptr};
    winrt::Windows::Graphics::Capture::Direct3D11CaptureFramePool m_framePool{nullptr};
    winrt::Windows::Graphics::Capture::GraphicsCaptureSession     m_session{nullptr};
    winrt::event_token                                             m_closedToken{};

    // Dimensions: set from m_item.Size() in Phase02, rounded-to-even in Phase03
    uint32_t m_sourceWidth  = 0;
    uint32_t m_sourceHeight = 0;
    uint32_t m_encodeWidth  = 0;
    uint32_t m_encodeHeight = 0;

    // NVENC
    HMODULE                     m_nvencDll        = nullptr;
    NV_ENCODE_API_FUNCTION_LIST m_nvencFuncs{};
    void*                       m_encoder         = nullptr;
    NV_ENC_PRESET_CONFIG        m_presetConfig{};
    NV_ENC_CONFIG               m_encodeConfig{};
    NV_ENC_OUTPUT_PTR           m_bitstreamBuffer = nullptr;

    // GPU resource registration/mapping
    NV_ENC_REGISTERED_PTR m_registeredResource = nullptr;

    const GUID               m_presetGuid   = NV_ENC_PRESET_P4_GUID;
    const NV_ENC_TUNING_INFO m_tuningInfo   = NV_ENC_TUNING_INFO_HIGH_QUALITY;
    const uint32_t           m_frameRateNum = 60;
    const uint32_t           m_frameRateDen = 1;

    // Capture-encode statistics
    int      m_wgcFramesTotal     = 0;
    int      m_capturedFrames     = 0;
    int      m_needMoreInputCount = 0;
    int      m_encodedPackets     = 0;
    uint64_t m_totalEncodedBytes  = 0;
    double   m_elapsedSeconds     = 0.0;

    enum class TerminationReason { Unknown, MaxFrames, TimeLimit, SourceLoss };
    TerminationReason m_terminationReason = TerminationReason::Unknown;

    // Collected IVF packets (from Phase15 + Phase16 drain)
    struct EncodedPacket {
        std::vector<uint8_t> bytes;
        uint64_t             timestamp;
    };
    std::vector<EncodedPacket> m_packets;
};

const char* NvencStatusName(NVENCSTATUS st);

} // namespace wgc_nvenc_gpu
